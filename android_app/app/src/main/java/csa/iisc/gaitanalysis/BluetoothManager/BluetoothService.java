...
//        mConnectedThread.write(out);
    ...
    private void connectionFailed() {
        ...
        BluetoothService.this.start(BluetoothService.this.isOther);
    }
...
//                e.printStackTrace();
    ...
    private class ConnectedThread extends Thread {
            ...
            while (true) {
                try {
                    if (InstanceVals.getExpectedDataType() == InstanceVals.possibleRecType.READ) {
                        if (InstanceVals.getReadlen() != 0) {
                            byte[] buffer = new byte[InstanceVals.getReadlen()];
                            int rb = mmInStream.read(buffer);
                            sendToUI(buffer);

                        }
                    } else if (InstanceVals.getExpectedDataType() == InstanceVals.possibleRecType.DATASTREAM) {
                        byte[] buffer = new byte[InstanceVals.getReadlen()];
                        int rb = mmInStream.read(buffer);
                        sendToUI(buffer);

                    } else if (InstanceVals.getExpectedDataType() == InstanceVals.possibleRecType.DUMP) {
        ...
        private void sendToUI(byte[] buffer) {
        ...
        private void sendToUI(ArrayList<Integer> arr_byte) {
            byte[] buffer;
            //send the data to UI
         ...
         //   InstanceVals.expectedDataType = InstanceVals.possibleRecType.DUMP;
