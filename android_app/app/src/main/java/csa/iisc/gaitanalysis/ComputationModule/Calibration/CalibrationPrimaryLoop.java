...
public class CalibrationPrimaryLoop {
    ...
    private static final String TAG = "Primary Loop";
    public static double[] j1_valf = new double[3];
    public static double[] j2_valf = new double[3];
    public static boolean cancelRunning = false;
    private static double pi = 3.14;

    public static List<double[]> runLoop(int numOfIterations, int sizeOfData, double[][] gyro_s_knee, double[][] gyro_s_shank,
                                         double[][] acc_s_knee, double[][] acc_s_shank, double[][] gyro_s_derv_knee, double[][] gyro_s_derv_shank) {
        System.out.println("Running the Primary Loop. numOfIter " + numOfIterations);
//        numOfIterations=2;
        ...
        if (PerformExperimentActivity.progress != null)
            PerformExperimentActivity.progress.setMax(numOfIterations);
        ...
        for (k = 0; k < numOfIterations - 1; k++) {
            ...
            if (PerformExperimentActivity.progress != null)
                PerformExperimentActivity.progress.setProgress(k);
            ...
            if (Double.isNaN(x_val[k][0])) {
                Log.e(TAG, "runLoop: ERROR NAN. Calibration Not Valid");
                break;
            ...
            // =================
            if (isDebug) {
                System.out.println("Going in the inside loop. Size of the data: "+sizeOfData);
            }
...
//                    break;
                ...
                if (isDebug) {
                    System.out.println(" g1_val is when i is " + i + ": ");
                    for (int ki = 0; ki < 3; ki++) {
                        System.out.print(g1_val[ki] + " ");
                    }

                    System.out.println();

                    System.out.println(" g2_val is: ");
                    for (int ki = 0; ki < 3; ki++) {
                        System.out.print(g2_val[ki] + " ");
                    }
    ...
    // Working Fine
    public static double[] jVal(double phi, double theta) {
    ...
    // Working Fine
    public static double calcEjacob(double phi1, double theta1, double phi2, double theta2, double g1x, double g1y,
    ...
    // Working Fine
    public static double[] calcJacobDx(double phi1, double theta1, double phi2, double theta2, double g1x, double g1y,
                                       double g1z, double g2x, double g2y, double g2z) {
                ...
                * Math.sin(theta1)))
                ...
                .sqrt((Math
                        .pow((g1y * Math.cos(phi1) * Math.cos(theta1)
                                - g1x * Math.cos(phi1) * Math.sin(theta1)), 2)
                        + Math.pow((g1x * Math.sin(phi1) - g1z * Math.cos(phi1) * Math.cos(theta1)), 2)
                        + Math.pow((g1y * Math.sin(phi1) - g1z * Math.cos(phi1) * Math.sin(theta1)), 2))));
                ...
                * (g2y * Math.sin(phi2) - g2z * Math.cos(phi2) * Math.sin(theta2)))
                ...
                .sqrt((Math
                        .pow((g2y * Math.cos(phi2) * Math.cos(theta2)
                                - g2x * Math.cos(phi2) * Math.sin(theta2)), 2)
                        + Math.pow((g2x * Math.sin(phi2) - g2z * Math.cos(phi2) * Math.cos(theta2)), 2)
                        + Math.pow((g2y * Math.sin(phi2) - g2z * Math.cos(phi2) * Math.sin(theta2)), 2))));
